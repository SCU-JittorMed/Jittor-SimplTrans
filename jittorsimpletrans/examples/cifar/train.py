...
class Config:
    ...
    batchsize = 128
    num_workers = 8
    log_name = "cifar_experiment.log"
    best_acc = 0
    best_epoch = 0
    alpha = 0.001
    mode = "vit"  # Options: "vit", "simpltrans" (with modes: "random", "diagonal", "identity")
    
    # Model parameters - adjusted for CIFAR-10
    image_size = 32
    patch_size = 4
    num_classes = 10
    dim = 64
...
def train_epoch(model, train_loader, criterion, optimizer, config):
    ...
    # 手动计算批次数 - Jittor CIFAR10训练集大小为50000
    cifar10_train_size = 50000
    num_batches = (cifar10_train_size + config.batchsize - 1) // config.batchsize
...
def test_epoch(model, test_loader, criterion, config):
    ...
    with jt.no_grad():
        # 手动计算批次数 - Jittor CIFAR10测试集大小为10000
        cifar10_test_size = 10000
        num_batches = (cifar10_test_size + config.batchsize - 1) // config.batchsize
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            if (batch_idx + 1) % 5 == 0 or batch_idx == num_batches - 1:
                print(f"评估进度: {batch_idx+1}/{num_batches} 批次 | 损失: {total_loss/(batch_idx+1):.4f} | 准确率: {100.0*correct/total:.2f}%")
